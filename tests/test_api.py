...
from .utils import convert_list_str_to_list_dict
...
class TestCrawlAPI:
    ...
    def test_get_crawl_details(self):
        ...
        assert convert_list_str_to_list_dict(json["queue"]) == [
    ...
    def test_crawl_same_domain_scopes(self):
        ...
        scopes = convert_list_str_to_list_dict(res.json()["scopes"])
        assert set(map(lambda x: x["domain"], scopes)) == {"example.com", "iana.org"}
