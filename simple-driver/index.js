...
const behaviorsAPI = process.env.BEHAVIOR_API_URL;
const behaviorMaxTime = Number(process.env.BEHAVIOR_RUN_TIME || 0) * 1000;

const screenWidth = process.env.SCREEN_WIDTH || 1280;
const screenHeight = process.env.SCREEN_HEIGHT || 1280;

const injectScript = fs.readFileSync("./media.js");
...
async function runBehavior(page, url) {
  try {
    if (!behaviorsAPI) {
      console.log('No behavior api -- skipping behaviors');
      return;
    }

    const behaviorQuery = behaviorsAPI + "/behavior?" + querystring.stringify({url});

    const resp = await fetch(behaviorQuery);
    ...
    await page.exposeFunction('pptLog', (text) => console.log(text));

    const behaviorPromise = page.evaluate(async () => {
      for await (const res of $WBBehaviorRunner$.autoRunIter()) {
        window.pptLog(res);
      }
    });

    if (behaviorMaxTime) {
      await Promise.race([behaviorPromise, sleep(behaviorMaxTime)]);
    } else {
